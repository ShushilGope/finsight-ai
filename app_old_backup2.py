...
def _render_architecture_svg(active_agent_name: str | None, completed: set[str], timings: dict[str, float]) -> str:
    ...
    svg_parts.append(f'''
        <style>
            .node-rect {{ transition: all 0.4s ease; }}
            .pulse {{ animation: archPulse 1.6s ease-in-out infinite; }}
            @keyframes archPulse {{
    ...
    for i, (full_name, key, label, sub) in enumerate(PIPELINE_STAGES):
        ...
        node_cls = "node-rect pulse" if is_active else "node-rect"
...
def _current_agent_from_logs(logs: list[AgentLog]) -> str | None:
    for log in reversed(logs):
        for full_name, _, _, _ in PIPELINE_STAGES:
    ...
    if st.session_state.result is not None:
        ...
        with tab_charts:
            if result.structured_analysis is None:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.plotly_chart(_scenario_chart(sa), use_container_width=True)
                with c2:
